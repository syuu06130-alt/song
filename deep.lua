...
local function playSong(notesTable, title, bpm)
    ...
    currentTask = task.spawn(function()
        ...
        local baseTempo = bpm or 120
        local timeScale = 60/baseTempo -- 基準テンポ
...
-- [1] 既存曲: Libra Heart (imaizumi) - 修正版
...
-- [5] 新規: Twinkle Twinkle Little Star (きらきら星)
local notes_twinkle = {
...
local Window = Rayfield:CreateWindow({
    Name            = "🎹 Virtual Piano Player v3.0",
    LoadingTitle    = "Piano Player 完全版",
    LoadingSubtitle = "全21曲収録 / 画像マッピング完全準拠",
...
-- 初期化メッセージ
Rayfield:Notify({
    Title = "🎹 Virtual Piano Player v3.0",
