-- ============================================================
-- 🎵 Gold Boombox Hub v4.0 - Complete Edition
-- 100 Songs + Bangladesh + Input ID + Hubs + GUIs | Rayfield
-- ============================================================
...
local function playById(id, label)
    if not boomboxSound then createBoombox() end
    local name = label or ("Custom ID: "..id)
    boomboxSound:Stop()
    boomboxSound.SoundId="rbxassetid://"..id
    boomboxSound.Volume=currentVolume
    task.wait(0.05); boomboxSound:Play()
    isPlaying=true; currentSongName=name
    setGlow(true)
    Rayfield:Notify({Title="♪ Playing",Content=name,Duration=5,Image="rbxassetid://4483345998"})
end
...
TabCtrl:CreateButton({
    ...
    Callback=function()
        ...
        if was then
            ...
            for _,s in ipairs(bangladeshSongs) do if s.n==last then playSong(s) break end end
        end
...
TabCtrl:CreateSection("Status")
TabCtrl:CreateLabel("100 Songs  •  Bangladesh Tab  •  Input ID Tab")
TabCtrl:CreateLabel("Hubs  •  GUIs  •  Gold Boombox Model")

-- ── 🇧🇩 Bangladesh ────────────────────────────────────────────
local TabBD = Window:CreateTab("🇧🇩 Bangladesh", 4483345998)
TabBD:CreateSection("Bangladesh Songs")
...
TabInput:CreateLabel("Enter any Roblox Music Asset ID below")
TabInput:CreateLabel("Then press ▶ Play to start")
...
TabInput:CreateButton({
    Name = "▶  Play Asset ID",
    Callback = function()
        if inputID == "" or inputID == nil then
            Rayfield:Notify({Title="⚠️ No ID",Content="Please enter an Asset ID first",Duration=3,Image="rbxassetid://4483345998"})
            return
        end
        -- IDの数字チェック
        if not inputID:match("^%d+$") then
            Rayfield:Notify({Title="❌ Invalid ID",Content="Numbers only! Got: "..inputID,Duration=4,Image="rbxassetid://4483345998"})
            return
        end
...
TabInput:CreateLabel("• IDはRoblox音楽アセットのIDを入力")
TabInput:CreateLabel("• 数字のみ (rbxassetid:// は不要)")
...
TabCred:CreateSection("v4.0 Complete Edition")
TabCred:CreateLabel("100 Songs | Bangladesh | Input ID")
TabCred:CreateLabel("7 Boombox Hubs | 10 Other Hubs | 6 GUIs")
...
Rayfield:Notify({
    Title   = "🎵 Gold Boombox Hub v4.0",
    Content = "100曲 + Bangladesh + Input ID 全て準備完了！",
    Duration = 6,
