-- ==============================================================
--   🎸 Libra Heart + Libra Heart 2 + Mini Songs
--   Virtual Piano Player  ―  Rayfield UI  [Enhanced v2]
--
--   ★ 新機能:
--     ① Libra Heart 2 (F# major / BPM=104 / 約40s) 追加
--        右手: PhraseA(F#F#C#C#BBbG#BbG#F#) + PhraseB(F#F#C#F#G#F#)
--        左手: 10〜11秒間隔ベースドローン (F#/C#/G# 順環)
--     ② 和音内メロディ強調 (+8ms リード)
--     ③ Speed 可変 (0.75x〜2.0x)
--     ④ Melody Only モード
-- ==============================================================
...
local function sitAt(obj)
    ...
    else for _,c in ipairs(obj:GetDescendants()) do if c:IsA("BasePart") then pos=c.Position; break end end
    end
...
local function setSpeed(s)
    speedMult = s
    local label = ({[0.75]="🐢 スロー",[1.0]="▶ 通常",[1.25]="⚡ やや速め",
                     [1.5]="⚡⚡ 速め",[2.0]="🚀 2倍速"})[s] or ""
    Rayfield:Notify({Title="⚡ Speed: "..s.."x",Content=label,Duration=2,Image="rbxassetid://4483345998"})
end
...
local function melodyOnly(notes)
    local out, i = {}, 1
    while i <= #notes do
        local baseT, best = notes[i].time, notes[i]
        local j = i + 1
        while j <= #notes and notes[j].time - baseT < 0.025 do
            if (PITCH[notes[j].key] or 0) > (PITCH[best.key] or 0) then best = notes[j] end
            j = j + 1
        end
        out[#out+1] = {time=baseT, key=best.key}; i = j
    end
    return out
end

-- ── 再生管理 ────────────────────────────────────────────────
local isPlaying, stopFlag, currentSong = false, false, ""
...
local function playSong(notes, title)
    ...
    local spd = speedMult
    Rayfield:Notify({Title="♪ "..title..(spd==1 and "" or " x"..spd),
        Content="演奏開始！ ("..#notes.." ノーツ)",Duration=3,Image="rbxassetid://4483345998"})
    task.spawn(function()
        local t0 = tick()
...
-- Db major (♭5) / 和音内メロディ最高音が+8ms先発火済み
...
-- ── Libra Heart 2 (imaizumi) [72 notes / 0〜40s] ─
-- Key: F# major  BPM: 104  Quarter = 0.577s
--
-- 構成 (4 cycle × 9.23s = 36.9s + ending):
--   ┌ Phrase A (10 quarter ≈ 5.77s) ────────────────────────
--   │  右手: F#5 F#5 C#5 C#5 B4 Bb4 G#4 Bb4 G#4 F#5
--   │  キー: Q   Q   *   *   7  ^   %   ^   %   Q
--   ├ Phrase B (6 quarter ≈ 3.46s) ─────────────────────────
--   │  右手: F#5 F#5 C#5 F#5 G#5 F#5
--   │  キー: Q   Q   *   Q   W   Q
--   └ 左手 (cycle head ≈ 10.5s ごと):
--       Cycle 0: F#4=$ / Cycle 1: C#4=! / Cycle 2: G#4=% / Cycle 3: F#4=$
-- Ending: ritardando F#5→F#4→C#4→G#4
local notes_libraHeart2 = {
    {time=0.000,key="Q"},{time=0.008,key="$"},{time=0.577,key="Q"},{time=1.154,key="*"},
    {time=1.731,key="*"},{time=2.308,key="7"},{time=2.885,key="^"},{time=3.462,key="%"},
    {time=4.038,key="^"},{time=4.615,key="%"},{time=5.192,key="Q"},{time=5.769,key="Q"},
    {time=6.346,key="Q"},{time=6.923,key="*"},{time=7.500,key="Q"},{time=8.077,key="W"},
    {time=8.654,key="Q"},{time=9.231,key="Q"},{time=9.239,key="!"},{time=9.808,key="Q"},
    {time=10.385,key="*"},{time=10.962,key="*"},{time=11.538,key="7"},{time=12.115,key="^"},
    {time=12.692,key="%"},{time=13.269,key="^"},{time=13.846,key="%"},{time=14.423,key="Q"},
    {time=15.000,key="Q"},{time=15.577,key="Q"},{time=16.154,key="*"},{time=16.731,key="Q"},
    {time=17.308,key="W"},{time=17.885,key="Q"},{time=18.462,key="Q"},{time=18.470,key="%"},
    {time=19.038,key="Q"},{time=19.615,key="*"},{time=20.192,key="*"},{time=20.769,key="7"},
    {time=21.346,key="^"},{time=21.923,key="%"},{time=22.500,key="^"},{time=23.077,key="%"},
    {time=23.654,key="Q"},{time=24.231,key="Q"},{time=24.808,key="Q"},{time=25.385,key="*"},
    {time=25.962,key="Q"},{time=26.538,key="W"},{time=27.115,key="Q"},{time=27.692,key="Q"},
    {time=27.700,key="$"},{time=28.269,key="Q"},{time=28.846,key="*"},{time=29.423,key="*"},
    {time=30.000,key="7"},{time=30.577,key="^"},{time=31.154,key="%"},{time=31.731,key="^"},
    {time=32.308,key="%"},{time=32.885,key="Q"},{time=33.462,key="Q"},{time=34.038,key="Q"},
    {time=34.615,key="*"},{time=35.192,key="Q"},{time=35.769,key="W"},{time=36.346,key="Q"},
    {time=36.923,key="Q"},{time=37.788,key="$"},{time=38.654,key="!"},{time=39.519,key="%"},
}

-- ── きらきら星 (BPM=116 / 3verse) ───────────────────────────
local notes_twinkle = {
...
local notes_ode = {
    ...
    {time=18.750,key="4"},{time=19.000,key="3"},{time=19.500,key="2"},{time=20.000,key="1"},
    {time=20.500,key="2"},{time=21.000,key="5"},{time=22.000,key="3"},{time=22.500,key="3"},
    {time=23.000,key="4"},{time=23.500,key="5"},{time=24.000,key="5"},{time=24.500,key="4"},
    {time=25.000,key="3"},{time=25.500,key="2"},{time=26.000,key="1"},{time=26.500,key="1"},
    {time=27.000,key="2"},{time=27.500,key="3"},{time=28.000,key="2"},{time=28.750,key="1"},
    {time=29.000,key="1"},
}

-- ── Happy Birthday (3/4 / BPM=96) ───────────────────────────
...
local Win = Rayfield:CreateWindow({
    Name            = "🎸 Libra Heart I & II — Enhanced",
    LoadingTitle    = "Piano Player Enhanced v2",
    LoadingSubtitle = "Libra Heart 1 + 2 / Speed / Melody",
...
LT:CreateSection("🎶 Libra Heart (imaizumi) [Db major / 約225s]")

LT:CreateButton({ Name="▶  Full Play（フル演奏）",
    Callback=function() playSong(notes_libraHeart,"Libra Heart") end })
LT:CreateButton({ Name="▶  イントロ [0〜15s]",
    Callback=function()
        local p={} for _,n in ipairs(notes_libraHeart) do if n.time<=15 then p[#p+1]=n end end
        playSong(p,"Libra Heart [Intro]") end })
LT:CreateButton({ Name="▶  Part 1 [0〜63s]",
    Callback=function()
        local p={} for _,n in ipairs(notes_libraHeart) do if n.time<=63 then p[#p+1]=n end end
        playSong(p,"Libra Heart [Part 1]") end })
LT:CreateButton({ Name="▶  Part 2 [63〜120s]",
    Callback=function()
        local off,p=63,{} for _,n in ipairs(notes_libraHeart) do
            if n.time>=63 and n.time<=120 then p[#p+1]={time=n.time-off,key=n.key} end end
        playSong(p,"Libra Heart [Part 2]") end })
LT:CreateButton({ Name="▶  Part 3 [120〜180s]",
    Callback=function()
        local off,p=120,{} for _,n in ipairs(notes_libraHeart) do
            if n.time>=120 and n.time<=180 then p[#p+1]={time=n.time-off,key=n.key} end end
        playSong(p,"Libra Heart [Part 3]") end })
LT:CreateButton({ Name="▶  FINALE [180〜225s]",
    Callback=function()
        local off,p=180,{} for _,n in ipairs(notes_libraHeart) do
            if n.time>=180 then p[#p+1]={time=n.time-off,key=n.key} end end
        playSong(p,"Libra Heart [FINALE]") end })

LT:CreateSection("🎵 Melody Only モード")
LT:CreateButton({ Name="▶  Melody Only — Full",
    Callback=function() playSong(melodyOnly(notes_libraHeart),"Libra Heart [MELODY]") end })
LT:CreateButton({ Name="▶  Melody Only — Part 1",
    Callback=function()
        local p={} for _,n in ipairs(notes_libraHeart) do if n.time<=63 then p[#p+1]=n end end
        playSong(melodyOnly(p),"Libra Heart [MELODY P1]") end })

-- ══ 🎸2 Libra Heart 2 タブ ═══════════════════════════════════
local LT2 = Win:CreateTab("🎸② Libra Heart 2", 4483345998)

LT2:CreateSection("🎶 Libra Heart 2 (imaizumi) [F# major / BPM=104 / 約37s]")
...
LT2:CreateParagraph({
    ...
    Content =
        "Key: F# major  BPM: 104\n"..
        "━━━━━━━━━━━━━━━━━━━━\n"..
        "右手 Phrase A (約5.8s × 4回):\n"..
        "  F# F# C# C# B Bb G# Bb G# F#\n"..
        "  ↑下行する音型が特徴的\n\n"..
        "右手 Phrase B (約3.5s × 4回):\n"..
        "  F# F# C# F# G# F#\n"..
        "  ↑Aへ戻るブリッジフレーズ\n\n"..
        "左手 (約10.5s 間隔でドローン):\n"..
        "  Cycle1: F#4=$  Cycle2: C#4=!\n"..
        "  Cycle3: G#4=%  Cycle4: F#4=$\n\n"..
        "Ending: rit. F#5→F#4→C#4→G#4",
})

LT2:CreateButton({ Name="▶  Full Play（全曲 約37s）",
    Callback=function() playSong(notes_libraHeart2,"Libra Heart 2") end })
LT2:CreateButton({ Name="▶  Phrase A のみ (1回分 約5.8s)",
    Callback=function()
        local p={};
        local lim = 16 * (60/104)
        for _,n in ipairs(notes_libraHeart2) do
            if n.time < 10*(60/104)+0.1 then p[#p+1]=n end
        end
        playSong(p,"LH2 [Phrase A]") end })
LT2:CreateButton({ Name="▶  1〜2 cycle (約18.5s)",
    Callback=function()
        local lim = 2 * 16 * (60/104)
        local p={} for _,n in ipairs(notes_libraHeart2) do
            if n.time<=lim then p[#p+1]=n end end
        playSong(p,"LH2 [Cycle 1-2]") end })
LT2:CreateButton({ Name="▶  Melody Only",
    Callback=function() playSong(melodyOnly(notes_libraHeart2),"LH2 [MELODY]") end })

-- ══ 🎵 Mini Songs タブ ══════════════════════════════════════
local MT = Win:CreateTab("🎵 Mini Songs", 4483345998)

MT:CreateSection("童謡・クラシック (BPM正確版)")
MT:CreateButton({ Name="▶  きらきら星 (BPM=116 / 3verse / 約24s)",
    Callback=function() playSong(notes_twinkle,"きらきら星") end })
MT:CreateButton({ Name="▶  歓喜の歌 Ode to Joy (BPM=120 / A-B-A / 約29s)",
    Callback=function() playSong(notes_ode,"歓喜の歌") end })
MT:CreateButton({ Name="▶  Happy Birthday (3/4 BPM=96 / C6 サビあり / 約23s)",
...
SetT:CreateParagraph({
    ...
    Content=
        ...
        "LH2 使用キー (F# major):\n"..
        "  Q=F#5 *=C#5 W=G#5 7=B4 ^=Bb4 %=G#4\n"..
        "  $=F#4(bass) !=C#4(bass) %=G#4(bass)",
})
...
CT:CreateButton({
    Name="📊  状態確認",
    Callback=function()
        local s = "Speed: "..speedMult.."x"
        if isPlaying then
            Rayfield:Notify({Title="📊 再生中",Content="♪ "..currentSong.."\n"..s,Duration=2,Image="rbxassetid://4483345998"})
        else
...
CT:CreateParagraph({
    ...
    Content=
        ...
        "・黒鍵は自動でShiftキー処理済み\n"..
        "・和音内は最高音(メロディ)が8ms先に発火\n"..
        "・Melody Only: 同時刻の最高音のみ抽出\n"..
        "・Speed変更は次の演奏から反映",
})
